...
// .wrangler/tmp/bundle-NA9X2O/strip-cf-connecting-ip-header.js
...
app.get("/db-test", async (c) => {
  try {
    ...
    return c.json({ status: "DB Error", error: error.message }, 500);
  }
});
function generateMemoName(id) {
  return `memos/${id}`;
}
__name(generateMemoName, "generateMemoName");
...
__name(timestampToISO, "timestampToISO");
async function logActivity(db, creatorId, type, level = "INFO", payload, resourceId, resourceType) {
  try {
    await db.prepare(
...
app.post("/api/v1/auth/signup", async (c) => {
  try {
    ...
    }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("168h").sign(JWT_SECRET);
    const response = {
      user,
      accessToken: token
    };
...
app.post("/api/v1/memos", async (c) => {
  try {
    ...
    const insertId = result.meta.last_row_id || 0;
    await db.prepare(
...
app.get("/api/v1/memos", async (c) => {
  ...
  const memos = memos_result.results.map((memo) => ({
    id: memo.id,
...
app.delete("/api/v1/memos", async (c) => {
  ...
  if (memos.results.length !== ids.length) {
...
app.get("/api/v1/memos/search", async (c) => {
  ...
  const memos = search_results.results.map((memo) => ({
    id: memo.id,
    name: memo.name || generateMemoName(memo.id),
    uid: memo.uid || `memo-${memo.id}-${memo.creator_id}`,
    content: memo.content,
...
app.patch("/api/v1/system/setting", async (c) => {
  ...
  const settingsMap = [
    ...
    ["customized-profile", updates.customizedProfile ? JSON.stringify(updates.customizedProfile) : void 0],
...
app.post("/api/v1/resources", async (c) => {
  ...
  const resourceId = result.meta.last_row_id || 0;
  return c.json({
    id: resourceId,
...
app.get("/api/v1/resources", async (c) => {
  ...
  const resourceList = resources.results.map((r) => ({
    id: r.id,
    name: `resources/${r.id}`,
    filename: r.filename,
    type: r.type,
...
app.get("/api/v1/activities", async (c) => {
  ...
  const activityList = activities.results.map((activity) => ({
    id: activity.id,
  ...
  const response = { activities: activityList };
...
app.get("/api/v1/webhooks", async (c) => {
  ...
  const webhookList = webhooks.results.map((webhook) => ({
    id: webhook.id,
    name: webhook.name,
    url: webhook.url,
...
app.post("/api/v1/webhooks", async (c) => {
  ...
  const webhookId = result.meta.last_row_id || 0;
  await logActivity(
    db,
    user.id,
    "webhook.created",
...
// .wrangler/tmp/bundle-NA9X2O/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NA9X2O/middleware-loader.entry.ts
